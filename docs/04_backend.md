...
│   ├── framework-compile.ts   # 1ファイル → CommonJS。React / Vue / Svelte の各コンパイラ
...
│   ├── frameworks.ts          # React / Vue / Svelte の差分を集約（生成・編集・修復すべてがここを読む）
...
| `auditInteractivity()` | `outputKind === 'react' ? auditReact : auditHtml`。else 側は単一HTMLページ用の監査なので、**Vue と Svelte はインタラクション検査が丸ごと無効**（画面数・ルーター・既定ルート・共有ストアのどれも未検査） |
| `scoreHtml()` の分岐 | 同上の理由で **Vue と Svelte は単一HTML用のルーブリックで採点**。さらに配点側も `useReducer` `createContext` `createRoot` `useNavigation` `key={` `onSubmit=` と React の語彙で書かれており、同じ実装でも他フレームワークは加点されない。同一ブリーフでの実測: React 94 / Vue 75 / Svelte 56 → 修正後 94 / 84 / 68（残差は実際に不足しているアイコン・図版の分） |
...
modifyUI(instruction, html)
  ...
  ├─ 編集（directModify）
  │    プロジェクト（React / Vue / Svelte）で参照画像が無い → ファイル単位の編集（projectFileModify）
...
Vue と Svelte は **CSS を書いていなかったのではなく、React より多く書いていました**。
...
- `STYLESHEET_CONTRACT` を `stylesheetContract(kind)` に変更。クラス属性の綴りも、
  インラインスタイルの悪例も、フレームワークの構文で出します。Vue / Svelte には
...
唯一の実例として持つ、1つの定数でした。それが Vue と Svelte の生成にもそのまま
...
描画はしました）。**Vue と Svelte は `routes.ts` に `NAV_ITEMS` を宣言し、
...
これが「Vue と Svelte は品質が低い」の正体でした。
...
`.vue` も `.svelte` もありません。したがって Vue / Svelte プロジェクトでは、
...
v194 で、同じブリーフ・同じモデル・同じパイプラインなのに **React は11個、Vue と Svelte は
1個ずつ**しかコンポーネントを作りませんでした。`decomposition`・`icons`・`imagery-missing` の
3つが Vue と Svelte にだけ open で残るのも、これが理由です。
...
Vue と Svelte が毎ラウンド `imagery-missing` と `icons` を抱え、React だけが閉じていた
...
| `artFiles` | `illustrations/*.tsx` | Vue / Svelte では常に空 → `imagery-missing` が**永久に閉じない** |
...
報告されていました**。全部 Vue と Svelte です。修正後は0件。

`static-chart` は同じリストを読んでいるので、**Vue と Svelte のグラフを一度も見ていません**でした。
...
| `fixSvelteStorePrefix` | ルーンモジュールへの `$` 接頭辞（`e.subscribe is not a function`） |
| `fixSvelteRuneShadowing` | `state` という名前の export がルーンを覆う |
| `fixSvelteKitImports` | `$app/navigation`（SvelteKit ではないのに） |
| `fixSvelteLegacyMount` | `new App({target})`（Svelte 5 は `mount(App, …)`） |
| `fixSvelteRuneImports` | `import { $state } from 'svelte'`（ルーンは構文） |
| `fixSvelteLazyRuneState` | 関数の中で `$state()` を代入（遅延シングルトン） |
| `fixSvelteRuneInGetter` | getter から `$derived(...)` を return |
| `fixSvelteDerivedExport` | `export const x = $derived(...)`（読み手側も書き換え） |
| `fixSvelteDerivedThunk` | `$derived(() => …)()`（コールバック形は `$derived.by`） |
| `fixSvelteDuplicateProps` | `$props()` の複数回呼び出し（Vue の重複 defineProps と同型） |
| `fixSvelteConstBlock` | `{#const …}`（ブロックではなくタグ、`{@const …}`） |
| `fixSvelteAttributeShorthand` | `{width={size}}`（短縮記法と通常形の衝突） |
| `fixSvelteNestedButton` | `<button>` の中の `<button>`（Svelte だけが致命的） |
| `fixSvelteGlobalAtRule` | `:global(@media …)`（at-rule はセレクタではない） |
| `fixSvelteLegacyProps` | ルーンと混在する `export let`（Svelte 4 と 5 の取り違え） |
| `fixSvelteReservedProp` | `export let class: additionalClass = ''`（予約語のプロパティ） |
| `salvageUnparsableStyles` | 解析できない `<style>` はブロックだけ捨てる |
| `fixSvelteUndefinedShorthand` | `<svg {width}>` だが宣言は `size` のみ（コンパイルは通り実行時に落ちる） |
| `fixSvelteDerivedThunk` / `fixSvelteDuplicateAccessor` | `$derived(() => …)()` / 内部変数と同名の export 関数 |
| `fixSvelteRuneStoreSubscribe` | ルーンで書かれたストアを `.subscribe()` で読む利用側との橋渡し |
| `fixSvelteInPlaceSort` | 描画経路での `x.sort(…)`（`$state` を破壊し `state_unsafe_mutation`） |
| `fixSvelteRuneCalledAsFunction` | `$derived` の値を関数として呼ぶ（`$.get(...) is not a function`） |
...
| `fixSvelteRuneShadowLocal` | `const state = …` がルーンを覆い隠す（`$state` がストア購読になる） |
| `fixSvelteRuneInObjectLiteral` | オブジェクトのプロパティに置かれたルーン → getter |
| `fixSvelteOrphanEffect` | モジュール直下の `$effect`（所有者がなく `effect_orphan`） |
| `fixSvelteClassDirectiveExpression` | `class:{'name'}={…}`（名前はリテラルのみ） |
| `fixSvelteReactiveStatement` | `$: x = expr` → `$derived`（ルーンと混在不可） |
| `fixSvelteMarkupTypeAssertion` | マークアップ式の `!`（型は剥がされない） |
