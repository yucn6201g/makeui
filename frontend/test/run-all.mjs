/*
 * Runs every test in this directory. Mirrors backend/test/run-all.mjs.
 *
 * `npm test` used to be a hand-written `&&` chain naming each file, on both
 * sides of the repo. Tests added in the same week as their fix were left out of
 * it and CodeBuild ran none of them — including the one covering a code-editor
 * path that had been dead for every project generated since the transport
 * changed.
 *
 ...
 *
 *   node test/run-all.mjs            (from frontend/)
