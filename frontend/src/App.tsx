...
import { uiImagesForSend, captionsForSend, roomForImages, MAX_UI_IMAGES } from './utils/uiImages';
import { promptProblem, oversizedImages, MAX_IMAGE_BYTES } from './utils/requestLimits';
import { titleFromResult, isUnnamed } from './utils/projectTitle';
import { titleWidthEm } from './utils/titleWidth';
import { AuthProvider, useAuth } from './auth/AuthProvider';
import { ErrorBoundary } from './components/ErrorBoundary';
import { normalizePreset, PRESETS } from './utils/presets';
import { Preview, type DeviceKind } from './components/Preview';
import { isOutputKind, detectKind, type OutputKind } from './utils/frameworkCompile';
import { CodeEditor } from './components/CodeEditor';
import { AdminPanel } from './components/AdminPanel';
import { PromptTemplates, CHAT_SUGGESTIONS } from './components/PromptTemplates';
...
import { setActiveJob, getActiveJob, clearActiveJob, setActivePhases, getActivePhases } from './utils/activeJob';
import { toHalfWidth, toDigits } from './utils/halfWidth';
...
function MainApp({ project, onBackToProjects, onUpdateProject, fetchProjectPreview }: MainAppProps) {
  ...
  const directEdit = useDirectEdit({
    ...
    /* A hand edit is a version like any other, so the list has to hear about it
       — see the note in utils/versionOptions.ts. */
  ...
  useEffect(() => {
    if (result && !isGenerating) {
       ...
       *
       * Read out of the document rather than asked for — see utils/projectTitle.
  ...
  const handleSend = () => {
    ...
    // Refused here rather than by a 400 after the upload — see utils/requestLimits.ts.
    ...
    if (chatMode === 'plan') {
      ...
      setLoadedHtml(null);
      // One picture is a reference, several are content. See utils/uiImages.ts —
          ...
          */}
          <UsageMenu
            /*
              ...
              placeholder holds the space. See utils/displayName.ts.
            */
