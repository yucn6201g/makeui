import { splitHtmlToFiles, type VFile } from './virtualFs';
import { toProjectFiles } from './scaffold';
import { isCompilableProject } from './reactPreview';

interface ShareBuild {
  html: string | null;
  error: string | null;
}

/** What `buildReactPreview` does, named so this module can be tested without it. */
type BuildDocument = (files: VFile[]) => Promise<ShareBuild>;
