...
/**
 * The project scaffold moved to its own module.
 *
 * Re-exported because three components import it from here, and because the
 * split is about what the two files are *for* rather than about size: this
 * module reads a transported document apart into files, and that one wraps a
 * set of files in the manifest, config and readme that make them a project.
 * They were one function only because there was one framework.
 */
export { toProjectFiles } from './scaffold';
