...
/** Which drawings a project has, and which of them anything renders. */
export function artworkFiles(files: Map<string, string>, kind: OutputKind): string[] {
  const ext = FRAMEWORKS[kind].componentExt
  return [...files.keys()].filter((p) => p.startsWith(ART_DIR) && p.endsWith(ext))
}
