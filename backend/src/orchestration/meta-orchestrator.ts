...
/** Minimum gap between in-flight stream writes, matching the generate pipeline. */
...
import { fixupProject } from '../tools/framework-fixups.js'
import { applyPresetFoundation } from './preset-foundation.js'
...
async function projectFileModify(
   ...
   */
  const { normalizeReactExtensions, addMissingBarrels } = await import('../tools/react-bundle.js')
   ...
   */
  const { repairSyntax } = await import('./syntax-repair.js')
...
async function directModify(
   ...
   */
  const { isReactBundle } = await import('../tools/react-bundle.js')
...
async function runModify(
  ...
  try {
     ...
     */
    {
      const { moduleDefects } = await import('../tools/react-bundle.js')
     ...
     */
    let editedFacts: Awaited<ReturnType<typeof import('../tools/browser-verify.js').verifyInBrowser>> = null
     ...
     */
    let repairedFacts: Awaited<ReturnType<typeof import('../tools/browser-verify.js').verifyInBrowser>> = null
    if (BROWSER_VERIFY_ENABLED && effortProfile(options.effort).browserVerify && routePlan.needsDesignPass) {
      ...
      try {
        const { verifyInBrowser } = await import('../tools/browser-verify.js')
        const { auditRuntime } = await import('./runtime-audit.js')
        const { isReactBundle } = await import('../tools/react-bundle.js')
        const { declaredScreenIds } = await import('./interaction-audit.js')
        /**
    ...
    if (introduced.length > 0) {
      ...
      try {
        ...
        if (runtimeBaseline) {
          ...
          const { runtimeRegressions } = await import('./runtime-audit.js')
          after.push(
            ...(facts
              ? [
         ...
         */
        const { toRunnableDocument } = await import('../tools/react-bundle.js')
