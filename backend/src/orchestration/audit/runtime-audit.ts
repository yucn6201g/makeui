...
import { auditComposition } from '../presets/preset-composition.js'
import type { BrokenBox, ContrastFault, RuntimeFacts, VerifyFailure } from '../../tools/browser/browser-verify.js'
 ...
 */
function undefinedClassesAround(source: string, elements: string[]): { path: string; classes: string[] }[] {
  if (!source) return []
  const files = readProjectFiles(source)
