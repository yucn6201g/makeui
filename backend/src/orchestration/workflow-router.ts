...
export interface GeneratePlan {
  /** Ordered subset of DESIGN_SPECIALISTS to run. Never empty. */
  specialists: DesignSpecialist[]
  complexity: 'simple' | 'standard' | 'complex'
  /** One line, shown in the chat so the routing is visible rather than magic. */
  reason: string
  /** True when the router answered; false when the fallback rules decided. */
  routed: boolean
}

export type ModifyScope = 'visual' | 'content' | 'structural' | 'behaviour'
