/**
 * The generate pipeline: one request to a finished, checked document.
 *
 * generateUI runs the design phase, assembles the project, then scores, repairs
 * and verifies it. The plan mode is in plan.ts, the Bedrock calls in
 * model-calls.ts and the shared types in types.ts.
 */
import { resolveModelForPrompt } from '../../config/model-config.js'
import { getPresetConfig } from '../../config/agentcore-config.js'
import { saveOutput } from '../../services/output-storage.js'
import { saveDesignMemory } from '../../tools/agent/memory-tool.js'
import { searchDesignSystem } from '../../tools/agent/knowledge-base-tool.js'
import { withTokenLedger, logLedger, type TokenLedger } from '../../services/token-ledger.js'
import { RepairBudget, REPAIR_FLOOR } from '../repair/repair-budget.js'
import { applyDeterministicFixes, clampDecorativeShadows } from '../repair/deterministic-fixes.js'
import { weightedCount } from '../repair/defect-weight.js'
import { worthRepairing } from '../repair/repair-yield.js'

import { buildProjectFiles } from './build-files.js'
import { readProjectFiles, stylesheetOf, spliceStylesheet } from '../../tools/project/project-transport.js'
import { detectKind } from '../../tools/project/framework-compile.js'
import { auditSeedData } from '../audit/seed-data-audit.js'
import { fixupProject } from '../../tools/fixups/framework-fixups.js'
import {
  salvageUnparsableStyles,
  stubUnbuildableComponents,
  unbuildableFiles,
} from '../../tools/fixups/module-fixes.js'
import { logger } from '../../utils/logger.js'
import { replyWithOutcome, projectFileCounts } from '../prompts/reply-text.js'
import {
  extractRequirements,
  checkRequirements,
  requirementDefects,
  requirementsBlock,
  designRequirementsBlock,
...
import { singleImageCaption } from './supplied-images.js'
import { parseAttachment, attachmentDirective } from '../../utils/data-attachment.js'
...
import {
  ...
  summariseRuntime,
  runtimeRegressions,
} from '../audit/runtime-audit.js'
import { critiqueScreenshot } from '../audit/visual-critic.js'
import { verifyInBrowser, type VerifyFailure } from '../../tools/browser/browser-verify.js'
import {
  toRunnableDocument,
  ...
  missingExports,
} from '../../tools/project/react-bundle.js'
import { keepRejectedCandidate } from '../../utils/rejected-candidate.js'
import { repairSyntax, syntaxDefects } from '../repair/syntax-repair.js'
import { effortProfile } from '../../config/effort.js'
import { FRAMEWORKS } from '../../config/frameworks.js'
import {
  pickStockImages,
  stockImageInstructions,
  hasStockLibrary,
  repairStockUrls,
} from '../../tools/images/stock-images.js'
...
import { auditDesignSystem } from '../audit/design-system-audit.js'
import { snapRadii, snapMotion } from '../presets/preset-conformance.js'
import { shellRequirement } from '../presets/preset-composition.js'
import {
  applyPresetFoundation,
  snapFontFamilies,
  snapComponentSizes,
  measureComponentDrift,
...
import {
  ...
  presetMotion,
  presetConformance,
} from '../presets/design-presets.js'
import { DEFAULT_OUTPUT_KIND } from '../../config/frameworks.js'
...
import {
  ...
  projectContract,
  SCREEN_COMPLETENESS,
} from '../prompts/prompt-contracts.js'
import { scoreBreakdown, withoutRoutingChanges, withoutFiles, revertSuspects } from '../audit/scoring.js'
import { isModelUnavailable } from '../../utils/failure-message.js'
import { FinalOutput, GenerateUIOptions } from './types.js'
import {
  getRunDesignSwarm,
  STREAM_FLUSH_MS,
  stripFences,
...
import { summariseDesignDecisions } from './plan.js'
...
async function runGeneration(options: GenerateUIOptions, ledger: TokenLedger): Promise<FinalOutput> {
   ...
   * the time the build needs it. See orchestration/generate/requirements.ts for why it is
...
FORBIDDEN — every one of these makes the build fail:
  ...
  ✗ window.__App__ / window.__useApp__     → import App from './app/App.js';
...
` +
  ...
  if (outputKind === 'react') {
     ...
     * resolve `from './components/ui.js'`, an import of a DIRECTORY, and measured
     ...
     * also have to emit `export { default as Foo } from './Foo.vue.js'` correctly,
...
${finalHtml.slice(0, 90000)}`
  ...
  if (unbuildable.length > 0 && unbuildable.length <= 6) {
    try {
      const { repairFiles } = await import('../repair/repair-files.js')
  ...
  const repairProjectByFile = async (defects: InteractionDefect[]): Promise<string | null> => {
    const { planFileRepairs, repairFiles } = await import('../repair/repair-files.js')
