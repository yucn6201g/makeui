...
1. 画面の一覧（5つ以上）
   ...
   「ナビゲーション:」の行で、メニューに置く画面（何も選んでいなくても意味のある画面）と、
   操作から開く画面（詳細・入力・確認・完了・会計）を分けて書く。操作から開く画面は、前提
   （商品を選んだ・カートに入っている・注文した）が無いまま開かれたら何を出すかまで書く。
...
4. 状態（空・送信中・エラー）
   ...
   「読み込み中」は書かない。データは手元にあり、読み込みは起きない。
...
ナビゲーション: 在庫サマリ・備品一覧・承認キュー。備品詳細は一覧の行から、発注申請は詳細から品目を入れた状態で開く。
